...
#Filter only churners from the data set
data_cl = data_cl[which(data_cl$Churn=="Yes"),]

#Remove churn from the cluster dataset, because the feature is constant
#Furthermore remove notice_period as the value is constant for all churners
cluster_ds = data_cl[,.(Bill_shock, 
                                 Online_account, 
                                 Opt_In_Mail, 
                                 Opt_In_Post, 
                                 Opt_In_Tel, 
...
cluster_ds$Longitude = as.numeric(cluster_ds$Longitude)
...
cs_result[,.(Consumption, 
        ...
        Longitude,
        Latitude,
        Inhabitants,Cluster)] %>% drop_na() %>%
  gather(x, y, Consumption:Inhabitants) %>%
...
#Inhabitants
cs_result[Inhabitants > 0 & Inhabitants <= 35000 ] %>% drop_na() %>%
  ggplot(aes(x = Inhabitants, fill = Cluster, color = Cluster)) +
  geom_density(alpha = 0.5) +
  theme(axis.text.x = element_text(angle = 90, hjust = 1),
        legend.position = "top") +
  scale_color_tableau() +
...
ggplot(cs_result[which(cs_result$Cluster!=0),], aes(x=Cluster, y=Inhabitants)) + 
  geom_boxplot()+
...
cs_result[Longitude > 47 & Longitude <= 49 ] %>% drop_na() %>%
  ggplot(aes(x = Longitude, fill = Cluster, color = Cluster)) +
  geom_density(alpha = 0.5) +
  theme(axis.text.x = element_text(angle = 90, hjust = 1),
        legend.position = "top") +
  scale_color_tableau() +
...
ggplot(cs_result[which(cs_result$Cluster!=0),], aes(x=Cluster, y=Longitude)) + 
  geom_boxplot()+
...
cs_result[Latitude > 7 & Latitude <= 9 ] %>% drop_na() %>%
  ggplot(aes(x = Latitude, fill = Cluster, color = Cluster)) +
  geom_density(alpha = 0.5) +
  theme(axis.text.x = element_text(angle = 90, hjust = 1),
        legend.position = "top") +
  scale_color_tableau() +
...
ggplot(cs_result[which(cs_result$Cluster!=0),], aes(x=Cluster, y=Latitude)) + 
  geom_boxplot()+
